...
// A4E cannot copy data out of ASRAM. Rewrite partial batch views while all
// buffers still have generic shared scope, before InferSramScope maps the
// compact operands to ASRAM/WSRAM/RSRAM. This produces valid RSRAM-to-bank
// routes and gives layout inference a real compact allocation to physically
// pad, instead of merely changing the later tile-view type.
...
PrimFunc RunBatchViewFallback(PrimFunc f) {
  auto target = f->GetAttr<Target>(tvm::attr::kTarget);
  if (!target.defined() || !TargetIsSunmmio(target.value()))
    return f;
  ...
  arith::Analyzer analyzer;
  PrimFuncNode *fp = f.CopyOnWrite();
  CompactBatchViewRewriter rewriter(&analyzer);
  fp->body = rewriter(f->body);
  return f;
}
...
Pass LegalizeSunmmioBatchGemmViews() {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    return RunBatchViewFallback(std::move(f));
  };
  return CreatePrimFuncPass(pass_func, 0, "tl.LegalizeSunmmioBatchGemmViews",
                            {});
}
