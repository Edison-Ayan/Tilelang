...
| BG-004 | 缺少端到端数值证据 | 建立BF16、MX、partial Sunsim/Gem5矩阵 | BF16 22/22、MX 4/4、partial 2/2 |
| MX-001 | global MX直接进入WSRAM触发1024-byte segment alignment错误 | 强制经RSRAM staging，再进入WSRAM/ASRAM | MX 4/4通过 |
...
| REM-006 | 中 | MX和容量边界覆盖仍不完整 | shared rank-2 MX及更大shape可能有未知问题 | 补shared MX、batch>8、shape>64 cube及SRAM边界矩阵 |
...
2. 给 `tc.mma`/`tc.init` 增加runtime batch count SSA operand。
3. verifier检查 `1 <= B_runtime <= B_max`。
4. lowering把SSA值写入CSR `0x808`。
5. DMA按 `scf.for b < B_runtime` 逐plane搬运，未激活slab不读写。
6. 同一个ELF连续验证 `B_runtime=1/2/B_max` 和越界失败。
